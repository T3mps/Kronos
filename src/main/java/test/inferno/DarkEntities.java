...
public final class DarkEntities {
    ...
    private static void createEntities(Registry dark, MapModel mapModel) {
        ...
        dark.emplace(new Camera(), new Light(50), new Position(firstRoomCenter.x(), firstRoomCenter.y()));
        ...
        for (int y = 0; y < mapModel.height(); y++) {
            for (int x = 0; x < mapModel.width(); x++) {
                ...
                dark.emplace(new Map(), new Position(x, y), switch (mapModel.map()[y][x]) {
    ...
    private static Scheduler createSystems(Registry dark, AtomicReference<ArrowKey> pressedKey, MapModel mapModel, Screen screen) {
        ...
        scheduler.schedule(() -> {
            ...
            dark.view(Camera.class, Position.class).stream().forEach(r -> {
        ...
        scheduler.schedule(() -> {
            ...
            var light = dark.view(Light.class, Position.class).iterator().next();
            Position lightPosition = light.component2();
            int lightLumen = light.component1().lumen;
            // finds all map entities
            var map = dark.view(Map.class, Position.class);
        ...
        scheduler.schedule(() -> {
            // finds the camera
            var camera = dark.view(Camera.class, Position.class).iterator().next();
            // finds all map entities
            var map = dark.view(Map.class, Render.class, Position.class);
