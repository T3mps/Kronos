...
import net.acidfrog.kronos.inferno.Group;
import net.acidfrog.kronos.inferno.Registry;
import net.acidfrog.kronos.inferno.Scheduler;
import test.inferno.MapModelBuilder.MapModel;

public final class DarkEntities {

    // don't print Registry banner info on startup
    static {
    ...
    private static void createEntities(Registry registry, MapModel mapModel) {
        ...
        registry.create(new Camera(), new Light(50), new Position(firstRoomCenter.x(), firstRoomCenter.y()));
        ...
        for (int y = 0; y < mapModel.height(); y++) {
            for (int x = 0; x < mapModel.width(); x++) {
                ...
                registry.create(new Map(), new Position(x, y), switch (mapModel.map()[y][x]) {
    ...
    private static Scheduler createSystems(Registry registry, AtomicReference<ArrowKey> pressedKey, MapModel mapModel, Screen screen) {
        // creates the scheduler
        var scheduler = registry.createScheduler();
        ...
        scheduler.schedule(() -> {
            ...
            registry.view(Camera.class, Position.class).forEach(r -> {
                Position position = r.component2();
        ...
        scheduler.schedule(() -> {
            ...
            var map = registry.view(Map.class);
        ...
        scheduler.schedule(() -> {
            ...
            var light = registry.view(Light.class, Position.class).iterator().next();
            Position lightPosition = light.component2();
            int lightLumen = light.component1().lumen;
            // finds all map entities
            var map = registry.view(Map.class, Position.class);
        ...
        scheduler.schedule(() -> {
            // finds the camera
            var camera = registry.view(Camera.class, Position.class).iterator().next();
            // finds all map entities
            var map = registry.view(Map.class, Render.class, Position.class);
            ...
            scheduler.forkAndJoin(
                    ...
                    () -> renderMap(camera.component2(), visibleMap, false, screen),
                    // renders the visited map
                    () -> renderMap(camera.component2(), visitedMap, true, screen)
            );
    ...
    private static void setVisibleArea(Position areaCenter, int area, Iterator<Group.With2<Map, Position>> mapView, MapModel mapModel) {
        while (mapView.hasNext()) {
            // gets the next tile and the position in the map
            var tile = mapView.next();
            Position mapPosition = tile.component2();
    ...
    private static void renderMap(Position cameraPosition, Iterator<Group.With3<Map, Render, Position>> mapView,
                                  boolean isVisitedView, Screen screen) {
        while (mapView.hasNext()) {
            // gets the next tile, the position in the map, and the render component to use
            var tile = mapView.next();
            Position position = tile.component3();
            Render render = tile.component2();
