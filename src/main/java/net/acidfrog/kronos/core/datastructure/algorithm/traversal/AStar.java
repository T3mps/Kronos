...
public final class AStar {
    ...
    public static final <T extends Comparable<T>> List<Graph.Edge<T>> getShortestPath(Graph<T> graph, Graph.Vertex<T> start, Graph.Vertex<T> goal) {
        ...
        for (var v : graph.getVertices()) fScore.put(v, Integer.MAX_VALUE);
    ...
    private static <T extends Comparable<T>> int distanceBetween(Graph.Vertex<T> start, Graph.Vertex<T> next) {
        for (var e : start.getEdges()) if (e.getToVertex().equals(next)) {
            return e.getCost();
        }
    ...
    private static @Internal <T extends Comparable<T>> int heuristicCostEstimate(Graph.Vertex<T> start, Graph.Vertex<T> goal) {
        return +1;
    }

    private static <T extends Comparable<T>> List<Graph.Edge<T>> reconstructPath(Map<Graph.Vertex<T>, Graph.Vertex<T>> cameFrom, Graph.Vertex<T> current) {
