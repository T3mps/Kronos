...
import static net.acidfrog.kronos.core.lang.assertions.Asserts.*;
...
class TimSort<T> {
	...
	public void doSort (T[] a, Comparator<T> c, int lo, int hi) {
		...
		if (DEBUG) assertTrue(lo == hi, "lo == hi");
		mergeForceCollapse();
		if (DEBUG) assertTrue(stackSize == 1, "stackSize == 1");
	...
	static <T> void sort (T[] a, int lo, int hi, Comparator<? super T> c) {
		...
		if (DEBUG) assertTrue(ts.stackSize == 1, "stackSize == 1");
	}
	...
	private static <T> void binarySort (T[] a, int lo, int hi, int start, Comparator<? super T> c) {
		if (DEBUG) assertTrue(lo <= start && start <= hi, "lo <= start && start <= hi");
		if (start == lo) start++;
		...
		for (; start < hi; start++) {
			...
			int left = lo;
			int right = start;
			if (DEBUG) assertTrue(left <= right, "left <= right");
			/*
	...
	private static <T> int countRunAndMakeAscending (T[] a, int lo, int hi, Comparator<? super T> c) {
		...
		} else while (runHi < hi && c.compare(a[runHi], a[runHi - 1]) >= 0) runHi++; // Ascending
	...
	private void mergeAt (int i) {
		if (DEBUG) assertTrue(stackSize >= 2, "stackSize >= 2");
		if (DEBUG) assertTrue(i >= 0, "i >= 0");
		if (DEBUG) assertTrue(i == stackSize - 2 || i == stackSize - 3, "i == stackSize - 2 || i == stackSize - 3");

		int base1 = runBase[i];
		int len1 = runLen[i];
		int base2 = runBase[i + 1];
		int len2 = runLen[i + 1];
		if (DEBUG) assertTrue(len1 > 0 && len2 > 0, "len1 > 0 && len2 > 0");
		if (DEBUG) assertTrue(base1 + len1 == base2, "base1 + len1 == base2");
		...
		if (DEBUG) assertTrue(len2 >= 0, "len2 >= 0");
		if (len2 == 0) return;
	...
	private static <T> int gallopLeft (T key, T[] a, int base, int len, int hint, Comparator<? super T> c) {
		if (DEBUG) assertTrue(len > 0 && hint >= 0 && hint < len, "len > 0 && hint >= 0 && hint < len");
		int lastOfs = 0;
		int ofs = 1;
		...
		if (DEBUG) assertTrue(lastOfs < ofs, "lastOfs < ofs");
		...
		if (DEBUG) assertTrue(lastOfs == ofs, "a[base + ofs - 1] < key <= a[base + ofs]");
		return ofs;
	}
	...
	private static <T> int gallopRight (T key, T[] a, int base, int len, int hint, Comparator<? super T> c) {
		...
		if (DEBUG) assertTrue(lastOfs == ofs, "a[b + ofs - 1] <= key < a[b + ofs]");
		return ofs;
	}
	...
	private void mergeLo (int base1, int len1, int base2, int len2) {
		if (DEBUG) assertTrue(len1 > 0 && len2 > 0 && base1 + len1 == base2, "len1 > 0 && len2 > 0 && base1 + len1 == base2");
		...
		while (true) {
			 ...
			 */
			do {
				if (DEBUG) assertTrue(len1 > 1 && len2 > 0, "len1 > 1 && len2 > 0");
		...
		if (len1 == 1) {
			...
			if (DEBUG) assertTrue(len2 == 0, "len2 == 0");
			if (DEBUG) assertTrue(len1 > 1, "len1 > 1");
			System.arraycopy(tmp, cursor1, a, dest, len1);
		}
	}
	...
	private void mergeHi (int base1, int len1, int base2, int len2) {
		...
		while (true) {
			 ...
			 */
			do {
				if (DEBUG) assertTrue(len1 > 0 && len2 > 1, "len1 > 0 && len2 > 1");
		...
		if (len2 == 1) {
			if (DEBUG) assertTrue(len1 > 0, "len1 > 0");
			dest -= len1;
			cursor1 -= len1;
			System.arraycopy(a, cursor1 + 1, a, dest + 1, len1);
