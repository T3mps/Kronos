...
public final class CostVertex<T extends Comparable<T>> implements Comparable<CostVertex<T>> {

    private int cost = Integer.MAX_VALUE;
    private Graph.Vertex<T> vertex = null;

    public CostVertex(int cost, Graph.Vertex<T> vertex) {
        if (vertex == null)
    ...
    public boolean equals(Object e1) {
        if (!(e1 instanceof CostVertex)) return false;

        CostVertex<?> pair = (CostVertex<?>)e1;
     ...
     */
    @Override
    public int compareTo(CostVertex<T> p) {
